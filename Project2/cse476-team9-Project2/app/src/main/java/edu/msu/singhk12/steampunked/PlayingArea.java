...
public class PlayingArea {
    ...
    public void setThisPlayer(int num) {thisPlayer = num; }
    /**
     * Set the player turn.
...
//                onReleased();
                ...
                for (int j = 0; j < height; j++) {
                    for (int i = 0; i < width; i++) {
                        try {
                            if (gridpipes[i][j] != null)
                            {
                                boolean ok = cloud.saveGridPipeToCloud(user, password, gridpipes[i][j]);
                                if (!ok) {
                                    Log.i("failed save", "???????????????????????????????");
                                }
                            }
    ...
    public synchronized void loadXML()
    {
        new Thread(new Runnable() {
            ...
            public void run() {
                final Cloud cloud = new Cloud();
                List<edu.msu.singhk12.steampunked.cloud.model.Pipe> ok = cloud.loadPipe(user, password);
                if (ok != null)
                {
                    for (edu.msu.singhk12.steampunked.cloud.model.Pipe pipe : ok)
                    {
                        Pipe newPipe = new Pipe(sTB(pipe.getConnect0()), sTB(pipe.getConnect1()), sTB(pipe.getConnect2()), sTB(pipe.getConnect3()), getContext(), Integer.parseInt(pipe.getId()));
//                        newPipe.set(thisGame, Integer.parseInt(pipe.getX()), Integer.parseInt(pipe.getY()));
                        newPipe.setRotateAngle(Integer.parseInt(pipe.getRotation()));
                        newPipe.setPlayer(Integer.parseInt(pipe.getPlayer()));
                        add(newPipe, Integer.parseInt(pipe.getX()), Integer.parseInt(pipe.getY()));
                    }
                    Log.i("loadingpipe", "ssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss");
                }
                else
                {
                    Log.i("loading pipe", "??????????????????????????????????");
